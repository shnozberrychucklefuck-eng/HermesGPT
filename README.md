...
- Persistent config — `hermesgpt_config.json` (copy from `config.example.json`)
...
## Install
    ...
    git clone <your-repo-url> hermesgpt
    cd hermesgpt
    pip install -r requirements.txt
    cp config.example.json hermesgpt_config.json
...
Then set `base_url` to `http://127.0.0.1:8080/v1` in `hermesgpt_config.json` (the default).
...
| `config.example.json` | config template (copy to `hermesgpt_config.json`) |
| `requirements.txt` | dependencies |

## Notes

- `hermesgpt_config.json` is git-ignored so real API keys never get committed.
